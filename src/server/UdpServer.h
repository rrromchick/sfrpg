...
struct ClientInfo {
    ClientInfo(const sf::IpAddress& l_ip, const PortNumber& l_port, const sf::Time& l_time)
        : m_clientIP(l_ip), m_clientPORT(l_port), m_lastHeartbeat(l_time), m_heartbeatWaiting(false),
...
public:
    template <class T>
    UdpServer(void(T::*l_handler)(sf::IpAddress&, const PortNumber&, const PacketID&, sf::Packet&, UdpServer*), T* l_instance)
